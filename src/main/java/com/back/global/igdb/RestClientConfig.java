...
public class RestClientConfig {
    ...
    // ── API 경로 (사용자 요청 전용) ──────────────────────────────────
    private static final Duration API_CONNECT_TIMEOUT = Duration.ofSeconds(1);
    private static final Duration API_READ_TIMEOUT    = Duration.ofMillis(1500);

    // ── 배치 경로 (대용량 쿼리 전용) ─────────────────────────────────
    private static final Duration BATCH_CONNECT_TIMEOUT = Duration.ofSeconds(5);
    private static final Duration BATCH_READ_TIMEOUT    = Duration.ofSeconds(30);

    @Bean
    public RestClient apiIgdbRestClient(RestClient.Builder builder, IgdbProperties props) {
    ...
    @Bean
    public RestClient batchIgdbRestClient(RestClient.Builder builder, IgdbProperties props) {
        HttpClient httpClient = HttpClient.newBuilder()
